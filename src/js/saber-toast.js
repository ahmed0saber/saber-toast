...
class SaberToast {
    ...
    success(params) {
        const colorValue = getComputedStyle(document.body).getPropertyValue("--success-clr");
        ...
        this.iconBackground = getComputedStyle(document.body).getPropertyValue("--icon-success-bg")
        this.fire(params)
    }
    error(params) {
        const colorValue = getComputedStyle(document.body).getPropertyValue("--error-clr");
        this.color = colorValue;
        ...
        this.iconBackground = getComputedStyle(document.body).getPropertyValue("--icon-error-bg")
        this.fire(params)
    }
    warn(params) {
        const colorValue = getComputedStyle(document.body).getPropertyValue("--warn-clr");
        this.color = colorValue;
        this.icon = `<svg xmlns="http://www.w3.org/2000/svg" width="20" height="18" viewBox="0 0 20 18"><path id="Danger" d="M14.477,4.442l7.269,12.615a2.926,2.926,0,0,1,.25,1,2.76,2.76,0,0,1-.73,2.021,2.837,2.837,0,0,1-1.95.92H4.679a3.146,3.146,0,0,1-.91-.2,2.8,2.8,0,0,1-1.56-3.655L9.528,4.433a2.729,2.729,0,0,1,1.1-1.08A2.851,2.851,0,0,1,14.477,4.442Zm-1.61,8.314a.875.875,0,0,1-1.75,0v-2.8A.875.875,0,0,1,12,9.09a.866.866,0,0,1,.87.862ZM12,17.018a.873.873,0,1,1,0-1.745.867.867,0,0,1,.87.862A.878.878,0,0,1,12,17.018Z" transform="translate(-2 -3)" fill="${colorValue}" fill-rule="evenodd"/></svg>`
        this.iconBackground = getComputedStyle(document.body).getPropertyValue("--icon-warn-bg")
        this.fire(params)
    }
    info(params) {
        const colorValue = getComputedStyle(document.body).getPropertyValue("--info-clr");
        this.color = colorValue;
        this.icon = `<svg xmlns="http://www.w3.org/2000/svg" width="23" height="23" viewBox="0 0 23 23"><path id="Icon_ionic-ios-information-circle" data-name="Icon ionic-ios-information-circle" d="M14.875,3.375a11.5,11.5,0,1,0,11.5,11.5A11.5,11.5,0,0,0,14.875,3.375Zm1.05,16.808H13.813V12.216h2.112Zm-1.056-8.835a1.1,1.1,0,1,1,1.144-1.106A1.108,1.108,0,0,1,14.869,11.348Z" transform="translate(-3.375 -3.375)" fill="${colorValue}"/></svg>`
        this.iconBackground = getComputedStyle(document.body).getPropertyValue("--icon-info-bg")
        this.fire(params)
    }
    fire(params) {
